...
function UI.setTheme(name)
    if UI.Themes[name] then
        ...
        C = UI.Theme
        -- Fire theme change event to all registered controls
...
-- Register a control for config save/load
function UI.register(id, ctrl)
    UI._registry[id] = ctrl
end

-- Get all control states for config
...
function UI.initNotifications(parent)
    ...
    local layout = UI.listLayout(notifHolder, 6)
...
function UI.notify(title, msg, duration, notifType)
    ...
    mLbl.Size = UDim2.new(1, -24, 0, 26)
    mLbl.Position = UDim2.new(0, 16, 0, 26)
    ...
    local progress = Instance.new("Frame", n)
    ...
    -- Animate in
    UI.tween(n, {Size = UDim2.new(1, 0, 0, 62), BackgroundTransparency = 0.05}, 0.3)

    -- Progress drain
    UI.tween(progress, {Size = UDim2.new(0, 0, 0, 2)}, duration, Enum.EasingStyle.Linear)

    task.delay(duration, function()
        UI.tween(n, {Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1}, 0.4)
        task.delay(0.5, function()
...
function UI.toggle(parent, name, default, callback, order)
    ...
    -- Auto-register with parent page name + control name
